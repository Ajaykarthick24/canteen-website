...
// ✅ Check URL to decide which view to show
// If URL has /owner → show owner side
// Everything else → show student side
const isOwnerURL = window.location.pathname.includes("owner");
...
export default function App() {
  ...
  const catColor = (cat) => ({ meals: "#FFF3E8", snacks: "#EAF3DE", drinks: "#E6F1FB", desserts: "#FBEAF0" }[cat] || "#F5F4F0");
  const catEmoji = (cat) => ({ meals: "🍱", snacks: "🥪", drinks: "🥤", desserts: "🍮", all: "🍽️" }[cat] || "🍽️");
  const statusStyle = (s) => ({ pending: { bg: "#FFF3E8", color: "#C85A00", label: "Pending" }, preparing: { bg: "#E6F1FB", color: "#185FA5", label: "Preparing" }, ready: { bg: "#EAF3DE", color: "#3B6D11", label: "Ready ✓" } }[s] || { bg: "#eee", color: "#333", label: s });
  ...
  if (isOwnerURL) {
    return (
      <div style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: "100vh", background: "#F5F4F0", maxWidth: 480, margin: "0 auto" }}>
        ...
        <nav style={{ background: "white", borderBottom: "1px solid #E5E5E0", padding: "0 1rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: 54, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 16 }}>
              <span style={{ color: "#D85A30" }}>Canteen</span> Staff
            </span>
          </div>
          {user && (
            <button onClick={() => signOut(auth)} style={{ padding: "6px 14px", background: "#FCEBEB", border: "none", borderRadius: 8, fontSize: 13, cursor: "pointer", color: "#E24B4A", fontWeight: 600 }}>
              Logout
            </button>
          )}
        </nav>
        ...
        {!user ? (
          ...
          <div style={{ padding: "3rem 1.5rem 2rem" }}>
            ...
            <form onSubmit={handleLogin}>
              <div style={{ marginBottom: 14 }}>
                ...
                <input type="email" placeholder="owner@canteen.com" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)}
                  style={{ width: "100%", padding: "13px 14px", border: "1.5px solid #E5E5E0", borderRadius: 10, fontSize: 15, boxSizing: "border-box" }} />
              </div>
              <div style={{ marginBottom: 14 }}>
              ...
              <button type="submit" style={{ width: "100%", padding: 15, background: "#D85A30", color: "white", border: "none", borderRadius: 12, fontSize: 16, fontWeight: 700, cursor: "pointer", marginTop: 8 }}>
                Login
              </button>
            </form>
          </div>
          ...
          <div style={{ paddingBottom: 80 }}>
            ...
            <div style={{ display: "flex", background: "white", borderBottom: "1px solid #E5E5E0" }}>
              {[["orders", "📋 Live Orders"], ["manage", "⚙️ Manage Menu"]].map(([tab, label]) => (
              ...
              <div style={{ padding: "1rem" }}>
                {orders.length === 0
                  ? <div style={{ textAlign: "center", color: "#aaa", padding: "4rem 0" }}>
                      ...
                      <div style={{ fontSize: 14 }}>No orders yet. Waiting for students...</div>
                    </div>
                  : orders.map((order) => {
                    ...
                    return (
                      <div key={order.id} style={{ background: "white", border: "1px solid #F0F0EC", borderRadius: 14, padding: "1rem", marginBottom: 10, boxShadow: "0 1px 6px rgba(0,0,0,0.05)" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                          <div>
                            <div style={{ fontWeight: 700, fontSize: 15 }}>Order #{order.id.slice(-4).toUpperCase()}</div>
                            <div style={{ fontSize: 11, color: "#aaa" }}>{order.createdAt?.toDate?.().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }) || "Just now"}</div>
                          </div>
                          <button onClick={() => cycleStatus(order)} style={{ fontSize: 12, fontWeight: 700, padding: "7px 16px", borderRadius: 20, cursor: "pointer", border: "none", background: st.bg, color: st.color }}>
                            {st.label} ▾
                          </button>
                        </div>
                        ...
                        <div style={{ borderTop: "1px solid #F0F0EC", marginTop: 8, paddingTop: 8, display: "flex", justifyContent: "space-between", fontWeight: 800, fontSize: 15 }}>
                          <span>Total</span>
                          <span style={{ color: "#D85A30" }}>₹{order.total}</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            {ownerTab === "manage" && (
              <div style={{ padding: "1rem" }}>
                ...
                {menuItems.map((item) => (
                  <div key={item.id} style={{ background: "white", border: "1px solid #F0F0EC", borderRadius: 14, padding: "12px 14px", marginBottom: 10, display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
                      <div onClick={() => toggleAvail(item)} style={{ width: 42, height: 24, borderRadius: 24, background: item.available ? "#1D9E75" : "#ccc", position: "relative", cursor: "pointer", transition: ".2s" }}>
                      ...
                      <span style={{ fontSize: 10, color: item.available ? "#1D9E75" : "#aaa", fontWeight: 600 }}>{item.available ? "Available" : "Unavailable"}</span>
        ...
        {/* TOAST */}
        <div style={{ position: "fixed", bottom: 24, left: "50%", transform: `translateX(-50%) translateY(${toast ? 0 : 100}px)`, background: "#1A1A1A", color: "white", padding: "10px 20px", borderRadius: 24, fontSize: 13, fontWeight: 600, zIndex: 500, transition: "transform 0.3s", whiteSpace: "nowrap", opacity: toast ? 1 : 0 }}>
          ✓ {toast}
        </div>
      </div>
    );
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', sans-serif", minHeight: "100vh", background: "#F5F4F0", maxWidth: 480, margin: "0 auto", position: "relative" }}>
      ...
      <nav style={{ background: "white", borderBottom: "1px solid #E5E5E0", padding: "0 1rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: 54, position: "sticky", top: 0, zIndex: 100, boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
        ...
        <button onClick={() => setCartOpen(true)} style={{ position: "relative", background: "#D85A30", color: "white", border: "none", borderRadius: 10, padding: "8px 14px", fontSize: 20, cursor: "pointer" }}>
          🛒
          {cartCount > 0 && (
